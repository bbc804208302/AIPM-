...
情报链路：Source Registry → Source Adapter → Normalize → Deduplicate → Candidate Snapshot（最多 20）→ Agent PM Value Scoring → Opportunity Ranking → SignalFlow Intelligence Repository + Agent Run Repository。采集源故障必须隔离；默认运行必须是 dry-run，只有显式 `--write` 才允许更新 SignalFlow 每日候选快照。

Agent 评分链路：Latest Dual-track Candidates → `list_daily_signals` → `search_memory` → `score_candidates`（中文概述 + PM 价值类型 + 五维评分）→ `select_intelligence_for_pool`（全量排序）→ Agent Run Repository。深度分析链路：高分情报或人工选择 → `get_signal` → `search_memory` → LLM Decision → `create_demand_proposal` / `reject_signal` → Agent Run Repository → Human Review。用户界面统一使用“情报”，`signalId` 等英文名称仅作为内部兼容标识。不得保存或展示模型思维链，只保存输入证据、评分维度、工具调用摘要、最终结论与运行指标。
