...
export function CollectorTaskControls({
  ...
  async function runNow() {
    setBusy("run");
    setStatus("正在采集公开来源、准备情报并运行 Agent 机会评分…");
    try {
      ...
      const data = await response.json() as { error?: string; selected?: number; candidates?: number; failedSources?: number; scored?: number; autoAnalyzed?: number };
      if (!response.ok) throw new Error(data.error || "采集任务执行失败。");
      setStatus(`真实采集完成：准备 ${data.selected ?? 0} 条情报，Agent 评分 ${data.scored ?? 0} 条，自动深度分析 ${data.autoAnalyzed ?? 0} 条；来源候选 ${data.candidates ?? 0} 条，失败来源 ${data.failedSources ?? 0} 个。`);
      router.refresh();
    } catch (reason) {
  ...
  return (
    <section className="task-control-panel" aria-labelledby={`${track}-daily-task-title`}>
      ...
      <p className="control-note">{editable ? `该操作会真实请求已启用的${track === "domain" ? "业务领域" : "AI 行业"}来源、覆盖候选快照，并在本地已配置 LLM 时运行 Agent 机会评分和高分自动深度分析。` : "公开站点只展示任务状态，不允许访客修改计划或触发采集。"}</p>
    </section>
  );
}
