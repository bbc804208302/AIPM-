...
export function AgentTriageControls({
  ...
  async function runTriage() {
    ...
    try {
      ...
      const data = await response.json() as { error?: string; scannedSignals?: number; recommendations?: number };
      if (!response.ok) throw new Error(data.error || "自动初筛失败。");
      setStatus(`已扫描 ${data.scannedSignals ?? 0} 条情报，推荐 ${data.recommendations ?? 0} 条进入深度分析。`);
      router.refresh();
    } catch (error) {
  ...
  const disabledReason = !executable
    ...
    : !configured
      ? "本地尚未启用 Opportunity Agent，请先配置 LLM。"
      : signalCount === 0
        ? "暂无可供扫描的最新情报。"
        : "";
  ...
  return (
    <section className="agent-triage-control" aria-labelledby="agent-triage-control-title">
      <div>
        ...
        <h2 id="agent-triage-control-title">让 Agent 主动发现机会</h2>
        <p>扫描最近双轨情报，结合 Memory、业务价值与可行动性，自动推荐 Top 3。</p>
      </div>
      <div>
        ...
        <button className="brutal-control-button primary" type="button" disabled={disabled} onClick={runTriage}>
          <Radar size={15} />{busy ? "Agent 初筛中" : "运行今日自动初筛"}
        </button>
      </div>
